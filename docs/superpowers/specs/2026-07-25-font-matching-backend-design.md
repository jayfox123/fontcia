...
- **The ivfflat ANN index was dropped**, not kept. Built empty (before Task 6 populated real data), it caused severe under-recall once queried — up to a 7-row seed returning zero matches, confirmed via `EXPLAIN`. At this spec's own v1 scale (~100 rows) a sequential scan is exact and sub-millisecond, so the index was removed entirely (`fa591c1`) rather than worked around. If catalog scale ever grows enough to justify ANN search, re-add the index and reindex it against real data at that point — don't reintroduce it empty.
- **Real measured accuracy** (Task 8, full 100-font catalog, 0 errors): Top-1 31%, Top-3 45%, Top-5 56%. A center-crop preprocessing bug was found and fixed along the way (DINOv2's default preprocessing resizes-then-crops to a square, which was discarding ~87% of the original wide, short renders) — re-measuring after the fix barely moved these numbers (previously 30/49/52%), suggesting this is closer to DINOv2-as-fixed-feature-extractor's real ceiling on font discrimination among these 100 fonts than a preprocessing artifact. Whether this is "good enough" to build client wiring on is the open decision this spec always said would be made after seeing real numbers, not before.
